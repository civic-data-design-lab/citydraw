(function($window, L, d3, ClipperLib) {
    ...
    L.FreeDraw = L.FeatureGroup.extend({
        ...
        setMode: function setMode(mode) {
            ...
            if ( isCreate ){
                d3.select(document).on('touchstart',this.events.mouseDown)
                    .on('touchmove',this.events.mouseMove);
            }
        ...
        bindEvents: function bindEvents() {
            ...
            this.events = {
                ...
                mouseDown: function onMouseDown(event) {
                    ...
                    if ( event.type == "touchstart" && !$.contains(this.map._container, event.target) ) return;
                    this.map.on('mousemove', this.events.mouseMove);
                    this.map.on('mouseup', this.events.mouseUpNoMove);
                    if (this.creating) {
                        if ( event.type == "touchstart" ) this._createMouseMove(event);
                        return;
                    }
                    ...
                    if ( event.latlng )
                        ...
                        var m = d3.mouse( this.map._container );
                        this.fromPoint =  L.point(m[0],m[1]);
                    }
                ...
                mouseUpNoMove: function onMouseUpNoMove(event){
                    //console.log('up no move')
                    this.map.off('mousemove', this.events.mouseMove);
                    this.map.off('mouseup', this.events.mouseUpNoMove);
                    if ( this.creating ) this._createMouseMove(event);
                }.bind(this),
                ...
                mouseMove: function onMouseMove(event) {
                  //  console.log('move')
                    ...
                    this.map.off('mouseup', this.events.mouseUpNoMove);
                    this.map.on('mouseup', this.events.mouseUpLeave);

                    $(document).on('touchend',this.events.mouseUpLeave);
        ...
        _createMouseMove: function _createMouseMove(event) {
            ...
            if ( event.type == 'touchmove' || event.type == 'touchstart' ){
                var m = d3.mouse( this.map._container );
                point  = L.point(m[0],m[1]);
            } else {
                point = event.containerPoint || this.map.mouseEventToContainerPoint(event);
            }
